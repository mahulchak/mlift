...
void annotGaps(vector<mI> & cm, map<int,vq> & mRef, ccov & masterRef, ccov & masterQ, vector<mI> & cnv, map<int,vector<qord> > & umRef, string & refseq, string & qseq, vector<int> & seqLen,ofstream & fout)
{
	...
	for(unsigned int i=1; i< cm.size();i++)
	{
		...
		if(cnvmi.x1 != 0)
		{
			...
			cm[i].y1 = cnvmi.y2; // changing the interval so that it is not used for calling CNV again			
		}
...
//fout<<"CNV\t"<<cnvmi.rn<<"\t"<<cnvmi.x1<<" "<<cnvmi.x2<<"\t"<<cnvmi.qn<<"\t"<<cnvmi.y1<<"\t"<<cnvmi.y2<<endl;
//		}
		if(cm[i].y1>cm[i].y2)//inversion. generate the criteria for calling indels and CNVs
		{
			if(cm[i].y1 > cm[i].y2) //if previous was also inverted
			{
				//derive the condition for indels
